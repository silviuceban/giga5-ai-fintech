...
async function chatToMistral(input: string) {
...
app.post("/chat", jsonParser, async (req, res) => {
  ...
  const result = await chatToMistral(
    `{dataset:${JSON.stringify(transactionsData)}, message:${message}}`
  );
...
app.post("/transactions", jsonParser, async (req, res) => {
  ...
  const result = await chatToMistral(
    `{dataset:${JSON.stringify(transactionsData)}
    prompt: ${message}
    }`
  );
